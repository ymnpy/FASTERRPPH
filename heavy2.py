...
# These are mutated at runtime by _apply_theme(); initial values = dark theme.
BG         = PALETTE_DARK["BG"]
CARD       = PALETTE_DARK["CARD"]
BORDER     = PALETTE_DARK["BORDER"]
ACCENT     = PALETTE_DARK["ACCENT"]
ACCENT_HOV = PALETTE_DARK["ACCENT_HOV"]
FG         = PALETTE_DARK["FG"]
FG_DIM     = PALETTE_DARK["FG_DIM"]
FG_MONO    = PALETTE_DARK["FG_MONO"]
SUCCESS    = PALETTE_DARK["SUCCESS"]
WARN       = PALETTE_DARK["WARN"]

COLOR_SHELL      = PALETTE_DARK["COLOR_SHELL"]
COLOR_SHELL_EDGE = PALETTE_DARK["COLOR_SHELL_EDGE"]
COLOR_CBUSH      = "#3b82f6"
OPACITY_SHELL    = 0.35
...
class FastenerViewer(QMainWindow):
    ...
    def __init__(self):
        ...
        # theme must be set before _init_ui() builds any widgets
        self._theme = "dark"
    ...
    def _apply_theme(self, theme="dark"):
        global BG, CARD, BORDER, ACCENT, ACCENT_HOV, FG, FG_DIM, FG_MONO, SUCCESS, WARN
        ...
        COLOR_SHELL, COLOR_SHELL_EDGE = p["COLOR_SHELL"], p["COLOR_SHELL_EDGE"]
    ...
    def _restyle_all(self):
        ...
        self.log_hdr_lbl.setStyleSheet(f"font-weight:bold;font-size:10px;margin-top:4px;color:{FG};")
        ...
        if self.plotter:
            ...
            if self._shell_actor:
                try:
                    self._shell_actor.GetProperty().SetColor(pv.Color(COLOR_SHELL).float_rgb)
                    self._shell_actor.GetProperty().SetEdgeColor(pv.Color(COLOR_SHELL_EDGE).float_rgb)
                except Exception:
                    pass
            self.plotter.render()
    ...
    def _build_left(self):
        # Track widgets that need manual restyling on theme change
        self._browse_btns = []
        self._dim_labels  = []
