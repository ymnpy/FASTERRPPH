...
# =============================================================================
# BDF READER  (robust fallback cascade)
# =============================================================================
def my_read_bdf(path: str) -> BDF:
    """
    Try all combinations of punch/xref so that malformed or punch-format
    BDFs are still loaded.  Priority: punch+xref → punch+no-xref →
    no-punch+xref → no-punch+no-xref.
    """
    try:
        try:
...
def _clear_range_dynamic(ws, start_row: int, col_from: str, col_to: str):
    """Clear contents from start_row downward between col_from and col_to."""
    try:
...
def rf_joint_metal(wb, df_join: pd.DataFrame):
    """
    Write df_join to the Metallic Bolted Joint sheet, trigger Excel
    calculation, read back results.  Returns (sheet_name, df_out, elapsed).
    Skips ML branch entirely.
    """
    ...
    # Filter to cap elements, exclude composite mats, exclude fitting PIDs
    df_join = df_join[
    ...
    # THE TRICK — if every RF is ERROR, reset diameter and retry
    all_contain_error = (
    ...
    # Normalise RF column name so the viewer can always find "RF"
...
def rf_joint_composite(wb, df_join: pd.DataFrame):
    """
    Write df_join to the Composite Bolted Joint sheet, trigger Excel
    calculation, read back results.  Returns (sheet_name, df_out, elapsed).
    Skips ML branch entirely.
    """
    start = 9 if df_join.shape[0] != 0 else 11
    ...
    # Filter: prefer skin elements; keep only composite mats
    skin_rows = df_join[
    ...
    # THE TRICK — if every Failure Mode is ERROR, reset diameter and retry
    all_contain_error = (
...
class CalcThread(QThread):
    ...
    def run(self):
        ...
        try:
            ...
            # ── fastpph: element ID, component name, load case, loads ────
            ...
            # Columns we source exclusively from fastpph
            ...
            # Keep all columns that exist — rf_joint functions may need extras
            # (Element Type, MAT, W, T, Pin, Collar, Side, Diameter, etc.)
            df_fast["Element ID"] = pd.to_numeric(
                df_fast["Element ID"], errors="coerce").astype("Int64")

            # PBARL aliases expected by rf_joint functions
            ...
            # ── joint CSV: Fastener Name + Fastener Diameter only ────────
...
class FastenerViewer(QMainWindow):
    ...
    def _get_filtered_fastpph(self) -> "pd.DataFrame | None":
        """
        Return df_fastpph for the current load case selection.

        • A specific load case  → filter rows to that LC only.
        • "All"                 → compute per-element envelope: for each
          numeric column keep the value with the largest absolute magnitude
          (e.g. among -7, 5, 2, 4  → -7 wins; among -3, 2, 5, 9 → 9 wins).
          Non-numeric columns are taken from the row that has the envelope
          value for "Fx" (the primary shear load), falling back to first row.
        """
        ...
        # Primary column used to pick the governing row (largest |value|)
        ...
        for eid, grp in self.df_fastpph.groupby("Element ID"):
            ...
            # For every numeric column independently pick the extreme value
            # (the one with maximum absolute magnitude)
        ...
        # Tag so the status bar can reflect it
    ...
    def _recolor_cbush(self):
        ...
        # Remove any leftover legend from a previous discrete render
